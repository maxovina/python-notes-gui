...
class NoteGUI:
    def __init__(self, window):
        ...
        self.window.bind(
            "<Control-s>",
            lambda event: logic.SaveTextEditorContent(self.text_editor, self.sidebar),
        )
    ...
    def CreateMenu(self):
        ...
        file_menu.add_command(
            label="Save",
            command=lambda: logic.SaveTextEditorContent(self.text_editor, self.sidebar),
        )
        file_menu.add_command(
            label="Change Notes Folder", command=self.ChangeNotesFolder
        )
    ...
    def CreateWidgets(self):
        ...
        self.add_button = tk.Button(
            self.sidebar_frame,
            text="Add Note",
            font=config.FONT_HELVETICA,
            command=self.createNewNoteWidget,
        )
        self.sidebar_scroll = tk.Scrollbar(
            self.sidebar_frame, highlightbackground=config.FOREGROUND_COLOR
        )
        ...
        self.text_editor = tk.Text(
            ...
            width=70,
            height=30,
            font=config.FONT_COURIER,
            bg=config.BACKGROUND_COLOR,
            fg=config.FOREGROUND_COLOR,
            selectbackground="black",
            selectforeground="green",
            undo=True,
            yscrollcommand=self.text_editor_scroll.set,
            insertbackground=config.FOREGROUND_COLOR,
        )
    ...
    def createNewNoteWidget(self):
        answer = simpledialog.askstring(
            "New note", "Enter a name for your new note", parent=self.window
        )
        if answer:
